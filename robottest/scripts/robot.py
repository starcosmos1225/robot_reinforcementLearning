...
from costmap2local import GridMap
import subprocess
import signal
import os
import sys
    ...
    def _build_net(self,n_actions,load=False):
        ...
        with tf.variable_scope('actor'):
            image_l1 = tf.layers.conv2d(inputs=self.image, filters=6, kernel_size=5, strides=(1,1), padding = 'valid',
                                        activation=tf.nn.relu6, kernel_initializer=w_init, name='image_l1')
            image_p1 = tf.layers.max_pooling2d(inputs=image_l1, pool_size=(2, 2), padding = 'valid',strides=(2,2),name='image_p1')
            image_l2 = tf.layers.conv2d(inputs=image_p1, filters=6, kernel_size=5, strides=(1,1),padding = 'valid',
                                        activation=tf.nn.relu6, kernel_initializer=w_init, name='image_l2')
            image_p2 = tf.layers.max_pooling2d(inputs=image_l2, pool_size=(4, 4), padding='valid', strides=(4, 4),
                                               name='image_p2')
            image_p2f = tf.layers.flatten(image_p2)
            s1 = tf.concat([self.s, image_p2f], axis=1)
            if load==False:
                #w_init = tf.random_normal_initializer(0., .1)
                self.l_a = tf.layers.dense(s1, 200, tf.nn.relu6, kernel_initializer=w_init, name='la')
        ...
        with tf.variable_scope('critic'):
            ...
            s1 = tf.concat([self.s, image_p2f], axis=1)
            if load==False:
                self.l_c = tf.layers.dense(s1, 100, tf.nn.relu6, kernel_initializer=w_init, name='lc')
...
class Robot:
    ...
    def initEnv(self):
        ...
        x, y, goal.x, goal.y= genRandomWorld(path, 20)
...
def init():
    global pubAction, pubStopMovebase,gridMap
    rospy.init_node('mainController', anonymous=True)
    gridMap = GridMap(env, size=100)
